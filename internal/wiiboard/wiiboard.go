		...
		for _, e := range events {
			...
			case evdev.EV_SYN:
				...
				if w.calibratedWeight == 0 {
					...
					// validate of the ouptut: we need to have a constant weight to ensure that we don't end
					// up treating partial measure, where only some balances weight reported something wrongly
					if math.Abs(float64(curEvent.TopLeft+curEvent.TopRight+curEvent.BottomLeft+curEvent.BottomRight)-w.calibratedWeight)/w.calibratedWeight > 0.05 {
...
func (w *WiiBoard) Calibrate() {
	...
	for {
		...
		if newWeight < 1000 || math.Abs(float64(lastWeight-newWeight))/float64(newWeight) > 0.2 {
			topLeft = 0
			topRight = 0
			bottomRight = 0
			bottomLeft = 0
